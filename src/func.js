...
exports.createTeam = function(data, callback){
	...
	var pCheck = { $or : [ data.usr, { fname : {$in: []}, lname : {$in: []} } ]	};
	var pSet = {team : newTeam.name, side : newTeam.side};		
	for(var i=0; i<3; i++){
		if(i>0){
			var t = utils.trim(data.others[i-1]).split();
			pCheck.$or[1].fname.$in.push(t[0]);
			pCheck.$or[1].lname.$in.push(t[0]);
		}	
		var r = getSRand(3);
		newTeam.mem[i] = {};
		newTeam.mem[i].ava = (i==0) ? data.creator.ava : 'images/avatars/noplayer.png';
		newTeam.mem[i].spec = (i==0) ? data.creator.spec : ((r==0)?'OFFENSE':((r==1)?'DEFENSE':'SUPPORT'));
		newTeam.mem[i].info = (i==0) ? data.creator.info : '';
	}

	//---- check users
	users.find(pCheck, {_id:1, uname:1, fname:1, lname:1}).toArray(function(err, items){
		if(err){ 
			console.log('DB ERROR: ', err);
		...
		for(var i=0; i<3; i++){
			_ids[i] = items[i]._id;
			newTeam.mem[i].uid = items[i]._id;
			newTeam.mem[i].uname = items[i].uname;
			newTeam.mem[i].fname = items[i].fname;
			newTeam.mem[i].lname = items[i].lname
		}
		...
		users.update({_id: {$in : _ids}}, {$set:pSet}, UPDATE_OPT, function(err){
			...
			db.error(function(err, error){
				...
				teams.insert(newTeam, function(err, items){
					...
					callback(true, items[0]._id.toString());
				});
			});
		});
	});
...
exports.createTeamPrep = function(data, callback){
	...
	var result = {};	
	users.find(viewer, {uname:1, fname:1, lname:1}).toArray(function(err, items){
		if(err){
			console.log('DB ERROR: ', err);
			callback(false, DEF_ERR);
			return;
		...
		users.find({email: {$ne: data.email}, team: {$exists: false}, side: {$exists: false}}, {fname:1, lname:1}).toArray(function(err, items){
			if(err){
				console.log('DB ERROR: ', err);
				callback(false, DEF_ERR);
				return;
